...
def reconstruct_semantic_segments(
    ...
    for whisper_seg in whisper_segments:
        ...
        if not hasattr(whisper_seg, "words") or not whisper_seg.words:
            logger.warning("Segment has no word timestamps, using segment-level timing")
        ...
        for word_info in whisper_seg.words:
            ...
            if current_speaker is not None and word_speaker != current_speaker:
                ...
                if not current_segment.is_empty():
                    logger.debug(
                        f"Flushing segment due to speaker change: {current_speaker} -> {word_speaker}"
                    )
                    semantic_segments.append(
                        {
            ...
            if current_segment.should_flush(min_flush_length):
                logger.debug(
                    f"Flushing segment due to punctuation: {current_segment.get_text()[:50]}..."
                )
                semantic_segments.append(
                    {
                        "text": current_segment.get_text(),
            ...
            # Force flush if segment is too long
                ...
                logger.debug(
                    f"Flushing segment due to max length: {current_segment.get_length()} chars"
                )
                semantic_segments.append(
                    {
                        "text": current_segment.get_text(),
    ...
    logger.info(
        f"Reconstructed {len(semantic_segments)} semantic segments from word-level data"
    )
