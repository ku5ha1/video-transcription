...
from app.utils.cache import generate_query_hash, CACHE_PREFIX_CHAT, TTL_CHAT
...
"""
    ...
    def chat(
        ...
        try:
            ...
            segments = self.vector_service.search_segments(
                query=query, user_id=user_id, video_id=video_id, limit=limit
            )
    ...
    def _build_user_prompt_with_history(
        self, query: str, segments: List[dict], history: Optional[List[dict]]
    ) -> str:
