...
class MetadataService:
    ...
    def _resample_audio(
        self, audio_array: np.ndarray, orig_sr: int, target_sr: int = 16000
    ) -> np.ndarray:
    ...
    def detect_text_tone(self, text: str) -> str:
        ...
        try:
            ...
            result = self.tone_classifier(
                text, candidate_labels=self.tone_labels, multi_label=False
            )
    ...
    def get_metadata(
        self, text: str, audio_chunk: np.ndarray = None, sample_rate: int = None
    ) -> dict:
        """
