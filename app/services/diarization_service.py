...
class DiarizationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Sherpa-ONNX: {e}, using AssemblyAI fallback"
            )
    ...
    def get_speaker_labels(self, audio_path: str) -> list:
            ...
            try:
                ...
                try:
                    ...
                    if sample_rate != 16000:
                        ...
                        raise ValueError(
                            f"Sample rate must be 16000Hz, got {sample_rate}Hz"
                        )
        ...
        if not aai.settings.api_key:
            logger.warning(
                "ASSEMBLYAI_API_KEY not found. Falling back to alternating speakers."
            )
            return None
        ...
        try:
            ...
            if not transcript.utterances:
                logger.warning(
                    "No utterances found in transcript. Falling back to alternating speakers."
                )
                return None
