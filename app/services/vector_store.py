...
class VectorStoreService:
    ...
    def _segment_to_point(
        self, segment: TranscriptSegment, embedding: List[float]
    ) -> PointStruct:
    ...
    def search_segments(
        self, query: str, user_id: str, video_id: Optional[str] = None, limit: int = 5
    ) -> List[dict]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(results)} segments for user {user_id}"
                + (f" and video {video_id}" if video_id else "")
            )
            return results
