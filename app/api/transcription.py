...
from app.models.database import User, Video, VideoStatus
...
async def transcribe_video(
    ...
    try:
        ...
        minio_service.upload_file(
            content, object_name, file.content_type or "video/mp4"
        )
...
async def get_transcription_status(task_id: str):
    ...
    """
    logger.info("Status check requested", extra={"extra_fields": {"task_id": task_id}})  # nosec B608
...
async def stream_video(
    ...
    try:
        presigned_url = minio_service.get_file_url(
            video.minio_object_key, expires_in_seconds=3600
        )
