...
async def upload_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "upload.html", {"request": request, "user": current_user}
    )


@router.get("/video/{video_id}", response_class=HTMLResponse)
async def video_detail(
    request: Request, video_id: str, db: AsyncSession = Depends(get_db)
):
...
async def web_chat(request: Request, video_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await db.execute(
            select(ChatMessage)
            .where(
                ChatMessage.video_id == video_id, ChatMessage.user_id == current_user.id
            )
        ...
        user_message = ChatMessage(
            video_id=video_id, user_id=current_user.id, role="user", content=query
        )
        db.add(user_message)
        await db.commit()
...
async def index(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        "index.html", {"request": request, "user": current_user, "videos": videos}
    )
...
async def web_upload(
    ...
    result = await db.execute(
        select(Video).where(
            Video.user_id == current_user.id, Video.file_hash == file_hash
        )
    )
    ...
    if existing_video:
        logger.info(
            f"Duplicate file detected: {file_hash[:16]}... - linking to existing video: {existing_video.id}"
        )  # nosec B608
        return {
    ...
    task = process_video_task.delay(
        object_name, file.filename, str(current_user.id), str(video.id)
    )
...
async def web_stream_video(
    ...
    try:
        ...
        response = minio_service.client.get_object(
            minio_service.bucket_name, video.minio_object_key
        )
